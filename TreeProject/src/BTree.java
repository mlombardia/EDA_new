...
public class BTree<T extends Comparable<T>> implements ITree<T> {
    ...
    private T remove(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
            ...
            // internal node
            //Node<T> lesser = node.getChild(index); esto es para predecesor in-order
            Node<T> gt = node.getChild(index+1); //esto es para sucesor inorder

            //Node<T> greatest = this.getGreatestNode(lesser); esto es para predecesor inorder
            Node<T> lt = this.getLowestNode(gt); //esto es para sucesor inorder

            //T replaceValue = this.removeGreatestValue(greatest); esto es para predecesor inorder
            T replaceValue = this.removeLowestValue(lt); //esto es para sucesor inorder
            ...
            if (greatest.numberOfChildren() > maxChildrenSize) {
                this.split(greatest);
            }*/ //esto es para predecesor inorder
    ...
    /**
     * Remove lowest valued key from node.
     *
     * @param node
     *            to remove lowest value from.
     * @return value removed;
     */
